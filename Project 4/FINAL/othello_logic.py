...
class board_structure:
    rows = 0 #keeps track of initial rows
    columns = 0 #keeps track of initial columns
    current_player_move = '' #keeps track of player's moves
    win_condition = '' #keeps track of set win condition
    current_board = [] #holds the board
    black_list = [] #holds possible moves for black
    white_list = [] #holds possible moves for white
    white_count = 0 #keeps track of number of white pieces
    black_count = 0 #keeps track of number of black pieces
    ...
    def user_input(self) -> list:
        """ 
        Gets all the initial data of the board.
        """
        print('FULL')
        rows = int(input())
        columns = int(input())
    ...
    def initial_board_parser(self, u_list) -> list:
        """
        Creates the board from the initial data entered.
        """
        check_list = ['B', 'W', '.']
        new_list = []
